function net = adnet_train_RL_mot(net, all_vid_info, opts)
% ADNET_TRAIN_RL Train the ADNet with reinforcement learning
% 
% Sangdoo Yun, 2017.

% model from SL
load('./models/net_dummy_mot.mat'); % alg 1 line 1

% change layers around
net.addLayer('concat_ah',dagnn.Concat, {'x16_total', 'action_history'}, 'x16_ah_total');
net.layers(end+1) = net.layers(end);
net.layers(net.getLayerIndex('concat_patch')+2:end-1) = net.layers(net.getLayerIndex('concat_patch')+1:end-2);
net.layers(net.getLayerIndex('concat_patch')+1) = net.layers(end);
net.layers = net.layers(1:end-1);
net.rebuild();
for l = net.getLayerIndex('concat_ah'):numel(net.layers)
    if contains(net.layers(l).name, 'fc6')
        net.layers(l).inputs = {'x16_ah_total'};
    end
end
net.rebuild();
...
p = numel(net.params);
param_name = net.params(p).name;
while contains(param_name, 'fc6')
    if contains(param_name, '1f')
        net.params(p).value = 0.01 * randn(3,1,512,opts.num_actions,'single');
        net.params(p).value = cat(3, net.params(p).value, zeros(3,1,nvec,11));
    elseif contains(param_name, '2f')
        net.params(p).value = 0.01 * randn(3,1,512,2,'single');
        net.params(p).value = cat(3, net.params(p).value, zeros(3,1,nvec,2));
    end
...
for id = 1:3
    net.layers(net.getLayerIndex(sprintf('loss_id%d', id))).block.loss = 'softmaxlog_pg';
end
...
%         try
            ...
            for id = 1:3
                layer_name = sprintf('v%d_id%d', count, id);  
                fc_layer{id} = dummy_net.getLayer(layer_name);    
                net.params(net.getParamIndex(fc_layer{id}.params{1})).value(:,:,1:512,:) = ...
                    dummy_net.params(dummy_net.getParamIndex(fc_layer{id}.params{1})).value;
                net.params(net.getParamIndex(fc_layer{id}.params{2})).value = ...
                    dummy_net.params(dummy_net.getParamIndex(fc_layer{id}.params{2})).value;

                layer_name = sprintf('vs%d_id%d', count, id); 
                fc_layer_score{id} = dummy_score_net.getLayer(layer_name);    
                net.params(net.getParamIndex(fc_layer_score{id}.params{1})).value(:,:,1:512) = ...
                    dummy_score_net.params(dummy_score_net.getParamIndex(fc_layer_score{id}.params{1})).value;
                net.params(net.getParamIndex(fc_layer_score{id}.params{2})).value = ...
                    dummy_score_net.params(dummy_score_net.getParamIndex(fc_layer_score{id}.params{2})).value;
            end
...
%             net_conv.move('gpu');
%             net_fc.move('gpu');
%             obj_idx = net_fc.getVarIndex({'objective_id1', 'objective_id2', 'objective_id3'});        
%             pred_idx = net_fc.getVarIndex({'prediction_id1', 'prediction_id2', 'prediction_id3'});        
...
%             net_conv.vars(conv_feat_idx).precious = 1;
            ...
            vid_clip_starts = 1:size(vid_info.gt, 2) - RL_steps;
            vid_clip_ends = vid_clip_starts + RL_steps;
            vid_clip_ends(end) = size(vid_info.gt, 2);
            ...
            for clipIdx = 1:num_train_clips
                ...
                curr_bboxes = permute(vid_info.gt(:,frameStart,:), [1 3 2]);  % alg 1 line 4
...
%                 this_actions = zeros(num_show_actions, 1);
                ...
                for frameIdx = frameStart+1:frameEnd
                    ...
                    curr_gt_bboxes = vid_info.gt(:,frameIdx,:);
                    [curr_bboxes, action_history, ~, ims, actions, ah_onehot, ~] = ...
                        tracker_rl_mot(curr_img, curr_bboxes, net, action_history, opts);    % alg 1 line 8
                ...
                for id = 1:3
                    ...
                    if overlap_ratio(curr_gt_bboxes(id,:), curr_bboxes(id,:)) > 0.7 % TODO: get ratio for one box at a time
                        ah_pos{id} = cat(2, ah_pos{id}, ah_onehots(:,:,id));
                        imgs_pos{id} = cat(4, imgs_pos{id}, squeeze(imgs(:,:,:,id,:)));
                        action_labels_pos{id} = cat(1, action_labels_pos{id}, action_labels(id,:));
                    else
                        ah_neg{id} = cat(2, ah_neg{id}, ah_onehots(:,:,id));
                        imgs_neg{id} = cat(4, imgs_neg{id}, imgs(:,:,:,id,:));
                        action_labels_neg{id} = cat(1, action_labels_neg{id}, action_labels(id,:));
                    end              
                end
            end
...
%                     disp(net.getVarSizes());
        ...
        %                 fprintf('%d:\n%s\n',var,net.vars(var));
            ...
            fprintf('time: %04.2f, epoch: %02d/%02d, vid: %05d/%05d \n', ...
                toc(tic_train), epoch, opts.numEpoch, count, length(all_vid_info));

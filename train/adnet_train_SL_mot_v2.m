function [net, all_vid_info] = adnet_train_SL_mot_v2(opts, tail, fc4var, fc5var)
...
net = init_model_mot_v2(sprintf('./models/net_init_%s', tail), opts, fc4var, fc5var);
% return;
%
tic_train = tic;
...
%         net.params(net.getParamIndex(fc_layer.params{1})).value ...
%             = dummy_net.params(dummy_net.getParamIndex(fc_layer.params{1})).value;
%         net.params(net.getParamIndex(fc_layer.params{2})).value ...
%             = dummy_net.params(dummy_net.getParamIndex(fc_layer.params{2})).value;
%         
...
%         net.params(net.getParamIndex(fc_layer_score.params{2})).value = dummy_score_net.params(dummy_score_net.getParamIndex(fc_layer_score.params{2})).value;
        ...
        for j = 1 : min (numel(train_db{i}), opts.frameBatch)
            ...
            imgs = get_masked_image(train_img, bboxes);
            imgs = single(imgs);
            imgs = gpuArray(imgs);
...
%             net.setExecutionOrder(exec_order_action);
            ...
            for k = 1 : num_batches + 1
                ...
                for id = 1:3
                    ...
                    curr_loss_score = gather(net.getVar(['objective_score' num2str(id)]).value); 
                    avg_loss = avg_loss + sum(curr_loss) + sum(curr_loss_score);
                end
                avg_loss = avg_loss / 6;
...
% FINALIZE TRAINING
net.move('cpu');
% net.params(net.getParamIndex(fc_layer.params{1})).value = 0.01 * randn(1,1,fc5var,3*opts.num_actions,'single');
% net.params(net.getParamIndex(fc_layer.params{2})).value = zeros(1, 3*opts.num_actions, 'single');
% net.params(net.getParamIndex(fc_layer_score.params{1})).value = 0.01 * randn(1,1,fc5var,3*2,'single');

function my_net = modify_model_mot_v2(my_net, opts)
% MODIFY_MODEL 
% 
% Sangdoo Yun, 2017.
...
for i=1:numel(my_net.layers)
    if strcmp(my_net.layers{i}.type,'conv')
        ...
        if ~isfield(my_net.layers{i}, 'filtersLearningRate')
            my_net.layers{i}.filtersLearningRate = 4 ;
        end
        if ~isfield(my_net.layers{i}, 'biasesLearningRate')
            my_net.layers{i}.biasesLearningRate = 8 ;
        end

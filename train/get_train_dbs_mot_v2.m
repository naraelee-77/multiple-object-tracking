function train_db = get_train_dbs_mot_v2(vid_info, opts)
...
for train_i = 1 : numel(train_sequences)  
    ...
    for id_idx = 1:vid_info.nids
        gt_bbox = vid_info.gt(img_idx, :, id_idx);
        ...
        % TODO: mix up randomly
        examples = [pos_examples; neg_examples];
        action_labels = gen_action_labels(opts.num_actions, opts, examples, gt_bbox);
        score_labels = [2*ones(size(pos_examples,1),1); ones(size(neg_examples,1),1)];
        
        rand_order = randperm(size(examples,1));
        
        examples = examples(rand_order,:,:);
        action_labels = action_labels(:,rand_order,:);
        score_labels = score_labels(rand_order,:);
        
        train_db(train_i).bboxes(:,:,id_idx) = examples;
        train_db(train_i).labels(:,:,id_idx) = action_labels;
        train_db(train_i).score_labels(:,id_idx) = score_labels;
    end

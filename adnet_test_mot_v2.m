function [bboxes, t_sum, precisions] = adnet_test_mot_v2(net, vid_path, opts, folder_name, model_name, idnum,interval)
...
vid_info.gt = [];
vid_info.img_files = [];
img_files = dir(fullfile(vid_path, folder_name, '*.jpg'));
for i = 1 : numel(img_files)
    vid_info.img_files(i).name = fullfile(vid_path, folder_name, img_files(i).name);
end
gt_path = fullfile(vid_path,sprintf('gt_id%01d_n%01d.txt',idnum,interval));
...
if ~exist(fullfile(vid_path, model_name), 'dir')
    mkdir(fullfile(vid_path, model_name));
end
...
%         action_history_oh_old = action_history_oh_zeros;
    ...
    if opts.visualize == true
        clf(res_fig);
        imshow(curr_img);
        rectangle('Position', vid_info.gt(frameIdx,:), 'EdgeColor', [0,1,0], 'LineWidth', 2.0); % ground truth box
        rectangle('Position', curr_bbox,'EdgeColor', [0,0,1],'LineWidth',2.0); % generated box
        set(gca,'position',[0 0 1 1]);
        text(20,10,num2str(frameIdx),'Color','y', 'HorizontalAlignment', 'left', 'FontWeight','bold', 'FontSize', 30);
        hold off;
        drawnow;
        saveas(gcf, fullfile(vid_path, sprintf('%s_id%01d_n%01d', model_name, idnum, interval), img_files(frameIdx).name));
    end
...
% SAVE TO TXT
dlmwrite(fullfile(vid_path, sprintf('%s_id%01d_n%01d_bbox.txt', model_name, idnum, interval)), bboxes);

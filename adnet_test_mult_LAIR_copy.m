function [bboxes, t_sum, precisions] = adnet_test_mult_LAIR_copy(color, path, model_name, idnums)
...
if ~exist(fullfile(path,'bboxes'),'dir')
    mkdir(fullfile(path,'bboxes'));
end
...
%     vid_info.gt{i}(:,4:5)=vid_info.gt{i}(:,4:5)+10;
    vid_info.gt{i}(:,2)=vid_info.gt{i}(:,2)-12;
...
% init containers
bboxes = zeros([length(idnums), size(vid_info.gt{1})]);
...
%             % ============================
%             curr_bbox{i} = vid_info.gt{i}(frameIdx,2:5);
...
%         fig=figure;
        ...
        for i = 1:length(idnums)
            ...
            rectangle('Position', vid_info.gt{i}(frameIdx,2:5), 'EdgeColor', color(i,:), 'LineWidth', 2.0); % ground truth box
...
%         drawnow;
        saveas(gcf, fullfile(path, 'bboxes', sprintf('%04d.jpg', frameIdx)));
